...
// the crawler-class.
//
// Sorry, but most static functions in here are not comletely compatible anymore
// with the ones from version 0.65, SORRY !
// (only this static functions on its own, the crawler and its "public-mthods"
...
class PHPCrawlerUtils {
  ...
  // static function splitURL returns the url-parts port, host, protocol, path, file, query, domain
  ...
  // static function "nomrmalizes" an URL.
  ...
  static function find_links(&$source, &$target_array, $aggressive_mode, &$tags_to_extract, &$map_array)
  {
    phpcrawlerutils::findLinks($source, $target_array, $aggressive_mode, $tags_to_extract, $map_array);
  }
  
  // static function findLinks finds and returns links in a given
  ...
  static function findLinks(&$source, &$target_array, $aggressive_mode, &$tags_to_extract, &$map_array) {
  ...
  // static function buildURL() "reconstruncts" a full URL from given links relating
  // to the URL they were found in
  
  static function buildURL ($link, $actual_url, $url_parts_actual="") {

    // Important: static function has to return a FULL URL, ioncluing
    // the port !!
  ...
  // static function buildURLs() "reconstruncts" full URLs from given links relating
  // to the URL they were found in
  
  static function buildURLs($links, $actual_url)
  {  
    $url_parts_actual = phpcrawlerutils::splitURL($actual_url);
  ...
  // static function just builds an URL out of the single URL-parts like "protocol", "host", "path" ...
  // given in the array $url_parts
  
  static function rebuildURL ($url_parts) {
  ...
  // static function removes URLs from a list of URLs that dont have
  // the same host
  
  static function removeURLsToOtherHosts(&$links, &$actual_url) {
  ...
  static function removeURLsToOtherDomains(&$links, &$actual_url) {
  ...
  // This static function is important:
  ...
  static function addToArray(&$elements, &$mean_array, &$map_array, $extended_linkinfo) {
  ...
  // static function decides if the content of a page/file should be received,
  // depending on the preg_match(es) $follow_content_type
  
  static function decideFollow(&$header, &$follow_content_type) {
  ...
  static function decideStreamToMemory (&$header, &$match_array) {
  ...
  static function decideStreamToTmpFile (&$header, &$match_array) {
  ...
  // static function simply checks a header for a redirect-location and returns it
  
  static function getRedirectLocation(&$header) {
  ...
  // static function removes all URLs from an array ($links) that lead to a page/file
  // ABOVE the path of the base url
  
  static function removePathUpLinks(&$links, &$base_url)
  {
    @reset($links);
  ...
  // static function removes URLs from a list of urls that match the
  // expressions in $filter_matches
  
  static function removeMatchingLinks(&$links, &$filter_matches)
  {  
    @reset($links);
    while (list($key)=@each($links))
    {
  ...
  // static function removes URLs from a list of urls that DONT match the
  // expressions in $filter_matches (opposite of removeMatchingLinks)
  
  static function &removeNotMatchingLinks(&$links, &$filter_matches) 
  {
    @reset($links);
    while (list($key)=@each($links))
    {
  ...
  // static function checks a header for cookie-data and stores it in $cookie_array
  
  static function getCookieData(&$header, &$cookie_array, $host) {
  ...
  // static function searches the cookie_array for cookie_vars that should
  ...
  static function &buildHeaderCookieString (&$cookie_array, $current_host) {
  ...
  // static function finds the URL in a <base href... Tag if exists
  
  static function getBasePathFromTag (&$source) {
  ...
  // static function xtracts the HTTP-status-code from a header
  
  static function getHTTPStatusCode (&$header) {
  ...
  // static function gets a speacial "header-tag" form a header
  // ("Content-Type", "Content-Size" or whatever)
  
  static function getHeaderTag ($tag, &$header)
  {
  ...
  // static function checks if a multidimensional array build like
  // $a[0]["foo"]="bar"; $a[1]["foo"]="ahem"; ...
  // contains a special value for the keys "foo" (mdkey)
  
  static function inMDArray($array, $needle, $mdkey) {
  ...
  // static function adds the priority_level to a list of URLS
  ...
  static function addURLPriorities (&$url_array, &$priority_array) {
  ...
  // static function checks if an authentication-login (username, password) should be
  // send with a request for $url depending on the given authentications in
  // $authentication_array
    
  static function getAuthenticationForURL (&$authentication_array, $url)
  {
    $auth = array();
  ...
  // static function not used at the moment
  
  static function getPriorityLevelOffset ($array, $priority_level) {
  ...
  // static function checks if a given preg_pattern is a valid one.
  
  static function checkExpressionPattern($pattern)
  {
