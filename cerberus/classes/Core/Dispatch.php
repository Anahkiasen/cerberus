...
class Dispatch
{
  ...
  public static function assets($scripts = array())
  {
    ...
    foreach (self::$scripts as $key => $value) {
      ...
      // Account for unknown scripts
      if(!isset(self::$paths[$value]))
        self::$paths[$value] = array($value);
  ...
  public static function submodules($submodules)
  {
    ...
    foreach ($plugins as $plugin) {
      ...
      $loadedFiles = array_map('basename', $pluginFiles);

      // TODO : Find better way to check if a plugin is already loaded
      ...
      if(isset(self::$paths[$plugin]))
        foreach(self::$paths[$plugin] as $check)
          if(str::find('/plugins/', $check) and in_array(basename($check), $loadedFiles)) break 2;
      ...
      foreach ($pluginFiles as $key => $value) {
        ...
        if (file_exists($oldPath) and $newPath) {
          try {
  ...
  {
    return a::get(self::$CSS, 'url', array());
  }
  ...
  {
    return a::get(self::$JS, 'url', array());
  }
   ...
   *
   * @param string $modules A module to include
   */
  public static function addPHP($module)
  {
    $availableFiles =
      '{' .PATH_CORE. '{classes/*/,functions},' .PATH_COMMON. 'php}/' .
