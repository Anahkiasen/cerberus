...
class Navigation
{
  ...
  public static function content($include = false)
  {
      ...
      default:
        if(self::$filepath and file_exists(self::$folder.self::$filepath)) $filepath = self::$folder.self::$filepath;
          ...
          try {
            $error = str_replace(
              '{filepath}',
              self::$filepath. ' [' .self::$page. ']',
              l::get('error.filepath'));
    ...
    elseif($filepath) return $filepath;
    else return false;
  }
  ...
  private static function extension(&$page, &$sousPage)
  {
    ...
    if(!file_exists(self::$folder) and !SUBFOLDER) directory::make(self::$folder);
