...
class String
{
  ...
  */
  ...
  public static function sanitize($string, $type = 'str', $default = null)
  {
    ...
    switch ($type) {
      ...
      case 'upper':
        ...
        break;
      case 'words':
        $string = self::sanitize($string, 'plain');
  ...
  public static function split($string, $separator = ',', $length = 1)
  {
    ...
    foreach ($parts as $p) {
      $p = trim($p);
      if(self::length($p) > 0 and self::length($p) >= $length) $out[] = $p;
    }
  ...
  public static function truncate($string, $count = 255, $mode = null, $removeHTML = true, $trailing = null)
  {
    ...
    switch ($mode) {
      ...
      default:
        $string = str_replace("\n", ' ', $string);
        if(self::length($string) <= $count) return $string;
  ...
  public static function cutout($str, $length, $rep = '…')
  {
    ...
    // Cut and glue
    return self::substr($str, 0, $strlcenter) . $rep . self::substr($str, $strrcenter);
  }
  ...
  */
  ...
  public static function link($link, $text = false, $attr = null)
  {
    ...
    return '<a ' .trim($attributes). '>' . self::html($text) . '</a>';
  }
  ...
  */
  ...
  /**
   * A basic BBCode function
   *
   * @param  string  $content A text
   * @param  boolean $classes Whether to use CSS classes or plain HTML
   * @return return           A formatted text
   */
  public function bbcode($content, $css = true)
  {
    if (!$classes) {
      $content = preg_replace('#\[[gras|b]\](.+)\[/[gras|b]\]#isU', '<strong>$1</strong>', $content);
      $content = preg_replace('#\[[italique|i]\](.+)\[/[italique|i]\]#isU', '<em>$1</em>', $content);
      $content = preg_replace('#\[[souligne|u]\](.+)\[/[souligne|u]\]#isU', '<ins>$1</ins>', $content);
      $content = preg_replace('#\[titre\](.+)\[/titre\]#isU', '<h2>$1</h2>', $content);
      $content = preg_replace('#\[soustitre\](.+)\[/soustitre\]#isU', '<h3>$1</h3>', $content);
    } else {
      $content = preg_replace('#\[[gras|b]\](.+)\[/[gras|b]\]#isU', '<span class="b">$1</span>', $content);
      $content = preg_replace('#\[[italique|i]\](.+)\[/[italique|i]\]#isU', '<span class="i">$1</span>', $content);
      $content = preg_replace('#\[[souligne|u]\](.+)\[/[souligne|u]\]#isU', '<span class="u">$1</span>', $content);
      $content = preg_replace('#\[titre\](.+)\[/titre\]#isU', '<span class="h4">$1</span><br /><br />', $content);
    }

    $content = preg_replace('#\[image\](.+)\[/image\]#isU', str::img('$1'), $content);
    $content = preg_replace('#\[taille="(.+)"\](.+)\[/taille\]#isU', '<span style="font-size:$1">$2</span>', $content);
    $content = preg_replace('#\[couleur="(.+)"\](.+)\[/couleur\]#isU', '<span style="color:$1">$2</span>', $content);
    $content = preg_replace('#\[lien\](.+)\[/lien\]#isU', '<a href="$1">$1</a>', $content);
    $content = preg_replace('#\[lien=(.+)\](.+)\[/lien\]#isU', '<a href="$1">$2</a>', $content);
  ...
  {
    if(self::length($string) <= $chars) return $string;
    $string = self::substr($string, 0, ($chars - self::length($rep)));
    $punctuation = '.!?:;,-';
