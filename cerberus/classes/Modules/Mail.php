...
class Mail
{
  ...
  public function __construct($to, $subject, $content)
  {
    ...
    $this->messageRaw = String::unhtml($this->content);

    // If we have several recipients
    if(!is_array($to) and String::find(',', $to))
  ...
  public function send($header = null)
  {
    ...
    */
    ...
    try {
      ...
      if(!$status) throw new \Exception(Language::get('mail.error'));
    } catch (\Exception $e) {
  ...
  {
    String::status($this->send(), Language::get('mail.sent'), Language::get('mail.error'));
  }
  ...
  public static function flatten($array)
  {
    ...
    foreach ($array as $key => $value) {
      if(is_bool($value)) $value = String::boolprint($value);

      if (is_array($value)) {
        $return .= '<p><strong>' .forms::getReadable($key). '</strong> : <br />';
        foreach ($value as $k2 => $v2) {
          if(is_bool($v2)) $v2 = String::boolprint($v2);
